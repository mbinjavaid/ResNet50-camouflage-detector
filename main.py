...
# Create the base ResNet50 model (pre-trained on ImageNet), and don't include the fully-connected final head
baseModel = ResNet50(weights="imagenet", include_top=False, input_tensor=Input(shape=(224, 224, 3)))

# Creating a new head for the neural network:
headModel = baseModel.output
headModel = AveragePooling2D(pool_size=(7, 7))(headModel)
headModel = Flatten(name="flatten")(headModel)
headModel = Dense(256, activation="relu")(headModel)
headModel = Dropout(0.5)(headModel)
# There are two classes we want to predict:
headModel = Dense(len(config.CLASSES), activation="softmax")(headModel)

# Mount the new head onto the base model:
